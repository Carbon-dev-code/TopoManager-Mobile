// DbSchema.tsx
import { checkParcelle, Parcelle } from "../../../entities/parcelle";
import { Preferences } from "@capacitor/preferences";
import { DashboardStats } from "../../../entities/dashbord";
import { initDatabase } from "./Database";
import { Directory, Filesystem } from "@capacitor/filesystem";
import { checkPersonnePhysique, PersonnePhysique } from "../../../entities/demandeur/model/PersonnePhysique";
import {
  checkPersonneMorale,
  PersonneMorale,
...
export async function insertParcelle(parcelle: Parcelle): Promise<Parcelle> {
  try {
    ...
    const demandeursClean = parcelle.demandeurs.map((d: { id: string; type: number | null; representanType?: string | null; personnePhysique: { id: string }; personneMorale: { id: string } }) => ({
      id: d.id,
      type: Number(d.type),
    ...
    const riverinClean = parcelle.riverin.map((r: { repere: string | null; type: string; nom: string | null; typePersonne: 0 | 1 | null; personnePhysique?: { id: string } | null; personneMorale?: { id: string } | null; observation: string }) => ({
      repere: r.repere ?? "",
      type: r.type,
      nom: r.nom ?? null,
    ...
    const polygoneForStorage = (parcelle.polygone ?? []).map((pg: { toJSON?: () => unknown; points?: unknown[] }) =>
...
export async function getAllParcelles(page?: number, limit: number = 10,): Promise<{ data: Parcelle[]; total: number }> {
  try {
    ...
    const data: Parcelle[] = await Promise.all(
      docs.map(async (doc: { toJSON: () => Record<string, unknown> }) => {
        const raw = doc.toJSON();

        const demandeurs: Demandeur[] = await Promise.all(
          (raw.demandeurs ?? []).map(async (d: { id: string; type: number; representantType?: string; personnePhysiqueId?: string; personneMoraleId?: string }) => {
        ...
        const riverin: Riverin[] = await Promise.all(
          (raw.riverin ?? []).map(async (r: { repere: string | null; type: string; nom: string | null; typePersonne: number | null; personnePhysiqueId?: string; personneMoraleId?: string; observation: string }) => {
...
export async function insertPersonnePhysique(
  ...
  try {
    checkPersonnePhysique(personne);
    const database = await initDatabase();
