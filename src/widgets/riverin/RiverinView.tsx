...
import { Riverin } from "../../entities/parcelle";
  ...
  const cardContent = (
    <div className={`riverin-card ${getDirectionClass(riverin.repere)}`}>
      <div className="riverin-top">
        <div className="riverin-label">Riverain – {capitalize(riverin.repere)}</div>
      </div>
      <div className="riverin-body">
        <div className="riverin-name">
          <div className="riverin-name">
            {riverin.type === "autre"
              ? capitalize(riverin.nom)
              : riverin.typePersonne === 0
                ? `${riverin.personnePhysique?.prenom ?? ""} ${riverin.personnePhysique?.nom ?? ""}`.trim()
                : riverin.personneMorale?.denomination ?? ""}
          </div>
        </div>
        {riverin.observation && (
          <div className="riverin-note"> {capitalize(riverin.observation)} </div>
        )}
      </div>
    </div>
  );
